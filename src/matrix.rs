...
pub struct Vec_3d {
    ...
    y: f32,
    z: f32,
    w: f32, // for matrix vector multiplication
}
...
impl Vec_3d {
    ...
    fn dot_product(v1: &Vec_3d, v2: &Vec_3d) -> f32 {
        v1.x * v2.x + v1.y * v2.y + v1.z * v2.z
    }

    fn length(v1: &Vec_3d) -> f32 {
        Self::dot_product(&v1, &v1).sqrt()
    }

    fn normalize(v1: &Vec_3d) -> Vec_3d {
    ...
    fn intersect_plane(
        ...
        let plane_d = -Scene::vector_dot_product(&plane_n, &plane_p);
        let ad = Scene::vector_dot_product(line_start, &plane_n);
        let bd = Scene::vector_dot_product(line_end, &plane_n);
        let t = (-plane_d - ad) / (bd - ad);
        let line_start_to_end = Scene::vector_sub(line_end, line_start);
        let line_to_intersect = Scene::vector_mul(&line_start_to_end, t);
        Scene::vector_add(line_start, &line_to_intersect)
    }
...
impl Mat_4x4 {
    ...
    fn make_rotation_x(&mut self, angle_rad: f32) {
        self.m[0][0] = 1.0;
    ...
    fn make_rotation_y(&mut self, angle_rad: f32) {
    ...
    fn make_rotation_z(&mut self, angle_rad: f32) {
        self.m[0][0] = angle_rad.cos();
    ...
    fn make_translation(&mut self, x: f32, y: f32, z: f32) {
        self.m[0][0] = 1.0;
        self.m[1][1] = 1.0;
        self.m[2][2] = 1.0;
        self.m[3][3] = 1.0;
...
impl Scene {
    fn matrix_multiply_vector(m: Mat_4x4, i: Vec_3d) -> Vec_3d {
        let v = Vec_3d {
    ...
    fn matrix_multiply_matrix(m1: Mat_4x4, m2: Mat_4x4) -> Mat_4x4 {
    ...
    fn matrix_point_at(pos: &Vec_3d, target: &Vec_3d, up: &Vec_3d) -> Mat_4x4 {
        let mut new_forward = Self::vector_sub(target, pos);
        new_forward = Vec_3d::normalize(&new_forward);

        let a = Self::vector_mul(&new_forward, Self::vector_dot_product(up, &new_forward));
        let mut new_up = Self::vector_sub(up, &a);
        new_up = Vec_3d::normalize(&new_up);

        let new_right = Self::vector_cross_product(&new_up, &new_forward);
    ...
    fn matrix_quick_inverse(m: &Mat_4x4) -> Mat_4x4 {
        let mut matrix = Mat_4x4 { m: [[0.; 4]; 4] };
    ...
    fn vector_add(v1: &Vec_3d, v2: &Vec_3d) -> Vec_3d {
        Vec_3d {
            x: (v1.x + v2.x),
            y: (v1.y + v2.y),
            z: (v1.z + v2.z),
    ...
    fn vector_sub(v1: &Vec_3d, v2: &Vec_3d) -> Vec_3d {
        Vec_3d {
            x: (v1.x - v2.x),
            y: (v1.y - v2.y),
            z: (v1.z - v2.z),
    ...
    fn vector_mul(v1: &Vec_3d, k: f32) -> Vec_3d {
        Vec_3d {
            x: (v1.x * k),
            y: (v1.y * k),
            z: (v1.z * k),
    ...
    fn vector_div(v1: Vec_3d, k: f32) -> Vec_3d {
        Vec_3d {
            x: (v1.x / k),
            y: (v1.y / k),
            z: (v1.z / k),
    ...
    fn vector_dot_product(v1: &Vec_3d, v2: &Vec_3d) -> f32 {
        v1.x * v2.x + v1.y * v2.y + v1.z * v2.z
    }

    fn vector_length(v1: &Vec_3d) -> f32 {
        Self::vector_dot_product(&v1, &v1).sqrt()
    }

    fn vector_cross_product(v1: &Vec_3d, v2: &Vec_3d) -> Vec_3d {
        Vec_3d {
